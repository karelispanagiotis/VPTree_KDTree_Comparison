...
int testMPI( int    const n,
  ...
  float * const corpus = (float * ) malloc( n*d * sizeof(float) );
  
  if (id == 0) {                //============================== MASTER

    // ---------- Initialize data to begin with
    float const * const corpusAll = ralloc( n*d*p );
    ...
    for (int ip = 0; ip < p; ip++){
      ...
      MPI_Send(corpus, n*d, MPI_FLOAT, dst, tag, MPI_COMM_WORLD);

    } // for (ip)


    // ---------- Run distributed kNN
    knnresult const knnres = vptree_distrAllkNN( corpus, n, d, k);

    
    // ---------- Prepare global kNN result object
    knnresult knnresall;
    knnresall.nidx  = (int *)   malloc( n*p*k*sizeof(int)    );
    knnresall.ndist = (float *)malloc( n*p*k*sizeof(float) );
    knnresall.m = n*p;
    knnresall.k = k;
    ...
    for (int ip = 0; ip < p-1; ip++){
      ...
      MPI_Recv( knnres.ndist, n*k, MPI_FLOAT, rcv, tag, MPI_COMM_WORLD, &Stat);
    ...
    MPI_Recv(corpus, n*d, MPI_FLOAT, rcv, tag, MPI_COMM_WORLD, &Stat);
    ...
    MPI_Send(knnres.ndist, n*k, MPI_FLOAT, dst, tag, MPI_COMM_WORLD);
