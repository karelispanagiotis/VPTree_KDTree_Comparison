...
float distColMajor(float const * const X, float const * const Y,
                    int i, int j,
                    int d, int n, int m){
...
float distRowMajor(float const * const X, float const * const Y,
                    int i, int j,
                    int d, int n, int m){
...
int validateResult( knnresult knnres,
                    float const * const corpus, float const * const query,
  ...
  for (int j = 0; j < m; j++ ){
    ...
    for (int l = 0; l < k; l++ ){
      ...
      if (ap == COLMAJOR){
        ...
        float distxy = distColMajor( corpus, query, knnresnidx_cm(j,l), j, d, n, m );
        ...
        /* get distance to stored index */
        float distxy = distRowMajor( corpus, query, knnresnidx_rm(j,l), j, d, n, m );
